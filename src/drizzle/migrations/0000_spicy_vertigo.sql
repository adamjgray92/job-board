...
CREATE TYPE "public"."job_listing_applications_stage" AS ENUM('applied', 'interviewed', 'interested', 'hired', 'denied');--> statement-breakpoint
CREATE TABLE "users" (
...
--> statement-breakpoint
CREATE TABLE "organisation_user_settings" (
	"userId" varchar NOT NULL,
	"organsationId" varchar NOT NULL,
	"newApplicationEmailNotifications" boolean DEFAULT false NOT NULL,
	"minimumRating" integer,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "organisation_user_settings_userId_organsationId_pk" PRIMARY KEY("userId","organsationId")
);
--> statement-breakpoint
ALTER TABLE "job_listings" ADD CONSTRAINT "job_listings_organisationId_organisations_id_fk" FOREIGN KEY ("organisationId") REFERENCES "public"."organisations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "organisation_user_settings" ADD CONSTRAINT "organisation_user_settings_userId_users_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "organisation_user_settings" ADD CONSTRAINT "organisation_user_settings_organsationId_organisations_id_fk" FOREIGN KEY ("organsationId") REFERENCES "public"."organisations"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
